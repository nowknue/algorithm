...
public class BOJ16235_st {
	...
	public static void main(String[] args) throws IOException {
		...
		for( int i = 0; i < M ; i++) {
			st = new StringTokenizer(br.readLine());
			namu[Integer.parseInt(st.nextToken())-1][Integer.parseInt(st.nextToken())-1].add(Integer.parseInt(st.nextToken()));
		} // 나무 넣기
			...
			for ( int i = 0; i < N ; i++) {
				for ( int j = 0; j<N ; j++) {
					for ( int tree : namu[i][j]) {
						if(tree%5==0) {
							if(i>0 && j>0) {namu[i-1][j-1].addFirst(1);}
							if(i>0) namu[i-1][j].addFirst(1);
							if(i>0 && j<N-1) namu[i-1][j+1].addFirst(1);
							if(j>0) namu[i][j-1].addFirst(1);
							if(j<N-1) namu[i][j+1].addFirst(1);
							if(i<N-1 && j>0) namu[i+1][j-1].addFirst(1);
							if(i<N-1) namu[i+1][j].addFirst(1);
							if(i<N-1 && j<N-1) namu[i+1][j+1].addFirst(1);
						}
					}
				}
